...
(function () {
	...
	async function loadTracksForVersion(versionSelect, mediaSourceId) {
		...
		if (audioSelect) {
			...
			audioSelect.innerHTML = '<option selected disabled>Loading...</option>';
			buildCarouselFromSelect(audioSelect, 'audio');
		}
		if (subtitleSelect) {
			subtitleSelect.disabled = true;
			subtitleSelect.innerHTML = '<option selected disabled>Loading...</option>';
		...
		if (audioSelect) {
			audioSelect.innerHTML = ''; // Clear loading
			if (streams.audio && streams.audio.length > 0) {
				...
				audioSelect.innerHTML = '<option selected disabled>None</option>';
			}
		...
		if (subtitleSelect) {
			subtitleSelect.innerHTML = ''; // Clear loading
			if (streams.subs && streams.subs.length > 0) {
				...
				subtitleSelect.innerHTML = '<option selected disabled>None</option>';
			}
	...
	function checkAndRestore() {
		const form = document.querySelector('form.trackSelections');
		if (form) {
			if (!form._stcInitialized) {
				log('Restoring: form found but not initialized');
				initialize();
			} else {
				// Already initialized, but verify streams logic
				const versionSelect = form.querySelector('select.selectSource');
				const audioSelect = form.querySelector('select.selectAudio');
				// If version is selected but audio is empty/loading, retry
				if (versionSelect && versionSelect.value) {
					// safe to re-run loadTracksForVersion - it handles clearing/loading
					log('Restoring: refreshing tracks for version', versionSelect.value);
					loadTracksForVersion(versionSelect, versionSelect.value);
				}
			}
		}
	}
	...
	// Listen for navigation/view events
	document.addEventListener('viewshow', () => {
		log('ViewShow event detected');
		setTimeout(checkAndRestore, 200);
	});

	// Fallback for history navigation
