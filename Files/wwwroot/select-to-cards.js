...
(function () {
	...
	window.BaklavaStreamCache = window.BaklavaStreamCache || {};
	...
	async function fetchStreams(itemId, mediaSourceId) {
		...
		try {
			...
			if (response && response.cache) {
				if (!window.BaklavaStreamCache[itemId]) window.BaklavaStreamCache[itemId] = {};
				for (const [sid, data] of Object.entries(response.cache)) {
					window.BaklavaStreamCache[itemId][sid] = data;
				}
			...
			if (response && mediaSourceId) {
				if (!window.BaklavaStreamCache[itemId]) window.BaklavaStreamCache[itemId] = {};
				window.BaklavaStreamCache[itemId][mediaSourceId] = { audio: response.audio, subs: response.subs };
			}
	...
	async function loadTracksForVersion(versionSelect, mediaSourceId) {
		...
		// Check Cache First
		let streams = null;
		if (window.BaklavaStreamCache[currentItemId] && window.BaklavaStreamCache[currentItemId][mediaSourceId]) {
			streams = window.BaklavaStreamCache[currentItemId][mediaSourceId];
